  ...
  async execute() {
      ...
      while ( attempts > 0) {
          ...
          try {
              ...
              await this.helpersFunctions.waitForGasEVM(this.logger, `[Account ${this.addressIndex}][OrbiterBridge][toStarknet]`)
              let tx = {
              ...
              this.logger.error(`[Account ${this.addressIndex}][OrbiterBridge][toStarknet]  - Error during execute: ${e}`);
              attempts--
              if (attempts > 0) {
                  this.logger.info(`[Account ${this.addressIndex}][OrbiterBridge][toStarknet]  - Retrying... (${attempts} attempts left)`);
                  await this.helpersFunctions.setupExactDealay(General.delayBeforeNextRetry, `[Account ${this.addressIndex}][OrbiterBridge][toStarknet] `, this.logger)
              } else {
                  this.logger.error(`[Account ${this.addressIndex}][OrbiterBridge][toStarknet]  - Maximum retry count reached. Stopping retries.`);
                  break
              }
          }
      }
