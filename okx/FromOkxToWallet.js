...
import {General, OKX, OKXAuth, ProviderMapping} from '../setting/config.js';
...
export default class FromOkxToWallet  {
   ...
   async execute(){
       ...
       while (attempts > 0) {
            try {
                await this.helperfunctions.networkAbility(this.networkAndPrivateKeyAddress.network, this.logger, `[Account ${this.addressIndex}][withdrawalFromOkx]`, 'w')
                ...
                this.logger.error(`[Account ${this.addressIndex}][withdrawalFromOkx] - Error during execute: ${e}`);
                attempts--
                if (attempts > 0) {
                    this.logger.info(`[Account ${this.addressIndex}][withdrawalFromOkx] - Retrying... (${attempts} attempts left)`);
                    await this.helperfunctions.setupExactDealay(General.delayBeforeNextRetry, `[Account ${this.addressIndex}][withdrawalFromOkx]`, this.logger)
                } else {
                    this.logger.error(`[Account ${this.addressIndex}][withdrawalFromOkx] - Maximum retry count reached. Stopping retries`);
                    break
                }
